...
для отображения и хранения графа и фиксирует предварительный выбор. Требования
с идентификаторами и методами проверки разрабатываются на этапе требований,
полная модель данных и программные интерфейсы — на этапе проектирования.
...
- Время построения подграфа глубиной 3 — не более 2 секунд
  ([раздел 03](03-architecture.md), §3.7).
- Для изображения подграфа действуют ограничения глубины и числа узлов:
  по умолчанию глубина 3 и не более 500 узлов. Расчёт влияния для обслуживания
...
- Подграфы кэшируются по ключу `(root_id, параметры)`; кэш сбрасывается
  при изменении связей. Для согласования и начала работ проверяется актуальность
...
**Этап требований.** Выделить из §6.4–6.12 отдельные требования с префиксом `ГРФ`,
источником и методом проверки; закрепить параметры фильтров и ограничения.

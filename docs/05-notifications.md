...
Требования с идентификаторами и методами проверки разрабатываются на этапе
требований, полная модель данных и программные интерфейсы — на этапе проектирования.
...
| `incident.resolved` | Аварийное | Все получатели объявления | warning |
...
| Telegram (Bot API) | Ответ API подтверждает приём сообщения | Бот пишет только пользователю, который сам начал с ним диалог; не более 1 сообщения в секунду в один диалог и около 30 сообщений в секунду всего | Внешний сервис вне организации | Кандидат |
| Яндекс Мессенджер (Bot API Яндекс 360 для бизнеса) | Ответ API подтверждает приём сообщения | Бот пишет только пользователям своей организации, если личные сообщения не запрещены их настройками; нужна подписка Яндекс 360 | Облако Яндекса | Кандидат, если выбран Яндекс Трекер |
| VK WorkSpace (Bot API) | Ответ API подтверждает приём сообщения | Нужно подключение организации к VK WorkSpace | Облако VK или сервер организации | Резервный кандидат |
| Mattermost | Ответ API подтверждает приём сообщения | Нужен собственный сервер | Сервер организации | Кандидат, если сведения о ресурсах нельзя передавать внешним сервисам |
...
|---|---|---|
| Группировка | Одно событие даёт одному пользователю одно уведомление со списком всех его ресурсов. Проверка сроков создаёт одно событие на запуск, поэтому 15 истекающих лицензий попадают в одно уведомление | Alertmanager |
...
| Повторные попытки доставки | 3 попытки с задержкой 1, 5 и 25 минут |
...
| 4. Граф | По каким связям считается влияние при аварии. Преподаватель предложил связь «включает в себя»: всё, что входит в упавший ресурс, считается затронутым. Нужен признак полноты расчёта |
...
**Этап требований.** Выделить из §5.2–5.10 отдельные требования с префиксом `УВД`,
источником и методом проверки; закрепить параметры из §5.8.
